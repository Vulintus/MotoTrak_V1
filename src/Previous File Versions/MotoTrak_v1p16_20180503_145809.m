function MotoTrak_v1p16

%Compiled: 05/03/2018, 14:58:08
...
if nargin == 0                                                              %If there are no optional input arguments...
    ...
    handles.version = 1.16;                                                 %Set the MotoTrak program version.
    ...
    if ~isempty(handles.ardy) && handles.ardy.version < 200                 %If the Arduino sketch version is older than version 2.0...
        ...
        try                                                                 %Attempt to run the program.
            MotoTrak_Upload_Controller_Sketch(handles.ardy.port,...
                handles.msgbox);                                            %Call the function to upload a sketch to the controller.
        ...
        if handles.ardy.version >= 200                                      %If the controller code was updated...
...
%WAITING FOR TRIAL INITIATION ******************************************************************************************************
    while max(trial.mon_signal) < h.init && ...
        ...
        if N > 0                                                            %If new samples were found...
...
while fix(run) == 1                                                         %Loop until the user starts a session, runs the calibration, or closes the program.
    ...
    if N > 0                                                                %If there was any new data in the stream.     
        ...
        if strcmpi(handles.curthreshtype,'milliseconds/grams') || ...
...
inputs = {  'pull',                     1;...
            ...
            'touch',                    1;...
            'both',                     1;...
            'capacitive sensor',        4};                                 %Match the input devices to their stream input indices.
...
function [session, trial, N] = MotoTrak_Update_Monitor_Signal(handles,session,trial)
...
try                                                                         %Attempt to add new samples to the buffer.
    ...
    for i = 1:N                                                             %Step through each line of the temp variable.
...
if session.do_once == 1                                                     %If this was the first stream read...
    session.buffer(1:session.buffsize-N,2) = ...
        session.buffer(session.buffsize-N+1,2);                             %Set all of the preceding signal data points equal to the first point.           
    session.buffer(1:session.buffsize-N,3) = ...
        session.buffer(session.buffsize-N+1,3);                             %Set all of the preceding IR data points equal to the first point.    
    session.do_once = 0;                                                    %Set the checker variable to 1.
end
trial.mon_signal(1:end-N,:) = trial.mon_signal(N+1:end);                    %Shift the existing samples in the monitored to make room for the new samples.

new_samples = session.buffsize-N+1:session.buffsize;                        %Grab the indices for the new samples.
...
function MotoTrak_Upload_Controller_Sketch(port,msgbox)
...
if isdeployed                                                               %If this is deployed code...
    ...
    if isempty(prog_path)                                                   %If no location was found for the current m-file...
        [prog_path,~,~] = fileparts(which('MotoTrak.m'));                   %Check for the location of the collated MotoTrak m-file.
    end
end
