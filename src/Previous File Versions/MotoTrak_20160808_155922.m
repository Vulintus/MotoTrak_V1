...
% set(handles.popdevice,'callback',@ChooseDevice);                            %Set the callback for the device pop-up menu.
% set(handles.popvns,'callback',@MotoTrak_Select_Stimulation);                %Set the callback for the VNS pop-up menu.
set(handles.popstage,'callback',@MotoTrak_Select_Stage);                              %Set the callback for the stage pop-up menu.
% set(handles.editpos,'callback',@EditPosition);                              %Set the callback for the position editbox.
% set(handles.edithitwin,'callback',@EditHitWindow);                          %Set the callback for the hit window duration editbox.
% set(handles.editthresh,'callback',@EditHitThreshold);                       %Set the callback for the hit threshold editbox.
% set(handles.popunits,'callback',@ChooseThresholdType);                      %Set the callback for the threshold type pop-up menu.
% set(handles.editinit,'callback',@EditInitiationThreshold);                  %Set the callback for the initiation threshold editbox.
...
set(handles.feedbutton,'callback','global run; run = 3')                    %Set the callback for the Manual Feed button.
...
handles.session_dur = 30;                                                   %Set a suggested session duration, in minutes.
...
%         if handles.threshmin >= handles.threshmax
%            handles.threshadapt = 'static';                                 %Set threshadapt to static if we have reached max threshold
%         else
%             handles.threshadapt = 'median';                                 %Set threshadapt to median if we have not reached max threshold
%         end
    else    
