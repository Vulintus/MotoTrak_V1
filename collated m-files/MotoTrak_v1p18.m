function MotoTrak_v1p18

%Compiled: 12/31/2018, 20:24:18
...
if nargin == 0                                                              %If there are no optional input arguments...
    ...
    handles.version = 1.18;                                                 %Set the MotoTrak program version.
    ...
    if N > 0                                                                %If there was any new data in the stream.     
        ...
        if i(1) == 1                                                        %If the primary tab is selected...
            ...
            if ~isnan(handles.ceiling) && handles.ceiling ~= Inf            %If a ceiling is set for this stage...
                ...
                minmax_y = [min([1.1*min(signal(:,1)), -0.1*thresh(1)]),...
                max([1.1*max(signal(:,1)), 1.3*thresh(1)])];                %Calculate new y-axis limits.  
            end      
            if minmax_y(1) == minmax_y(2)                                   %If the top and bottom limits are the same...
                minmax_y = minmax_y(1) + [-1,1];                            %Arbitrarily add one above and below the constant value.
            end
            if ~strcmpi(handles.device,'water')                             %If this isn't the water reaching module...
                ylim(handles.primary_ax,minmax_y);                          %Set the new y-axis limits.
                temp = get(txt(1),'extent');                                %Grab the position of the threshold label.
                if temp(2) < minmax_y(1)                                    %If the bottom edge of the text is outside the bounds...
        ...
        if any(strcmpi(handles.curthreshtype,...
            ...
            minmax_y(1) = ir_minmax(1)-0.1*(ir_minmax(2)-ir_minmax(1));        %Calculate the lower end of the y-axis limits.    
            minmax_y(2) = max([1.1*max(signal(:,2)), 1.3*thresh(2)]);          %Calculate the upper end of the y-axis limits.
            if minmax_y(1) == minmax_y(2)                                         %If the top and bottom limits are the same...
                minmax_y = [-1,1];                                             %Arbitrarily add one above and below the constant value.
            end
            ylim(handles.secondary_ax,minmax_y);                               %Set the new y-axis limits.
        end
    end
