function MotoTrak_v1p25

%Compiled: 06/01/2021, 21:28:50
...
if nargin == 0                                                              %If there are no optional input arguments...
    ...
    handles.version = 1.25;                                                 %Set the MotoTrak program version.
...
%     if datenum(version('-date')) >= 737685                                  %If the version is 2019b or newer...     
%         [port, booth_pairings] = ...
...
while now < timeout                                                         %Loop for 10 seconds or until a reply is noted.
    fwrite(serialcon,'A','uchar');                                          %Send the check sketch version code to the Arduino board.
    if serialcon.BytesAvailable > 0                                         %If there's bytes available on the serial line...
...
v1p4_clear_stream(serialcon);                                               %Clear any data off the serial line.
...
v2p0_clear_stream(serialcon);                                               %Clear any data off the serial line.
fclose(serialcon);                                                          %Close the serial connection.
delete(serialcon);                                                          %Delete the serial object.
