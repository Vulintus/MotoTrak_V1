...
close all force;                                                            %Force close all open windows.
clc;                                                                        %Clear the command window.
use_serialport = 0;                                                         %Set a flag for whether or not to use the new 'serialport' function.
...
if nargin == 0                                                              %If there are no optional input arguments...
    ...
    handles.version = 1.27;                                                 %Set the MotoTrak program version.
...
if ~exist('use_serialport','var')                                           %If the use_serialport flag is undefined...
    use_serialport = 0;                                                     %Set the use_serialport flag to zero.
end
...
switch handles.device                                                       %Switch between the recognized devices.
    case 'lever'                                                            %If the current device is the lever...     
        if handles.ardy.version < 200                                       %If the controller microcode version is less than 2.00...
            ...
            handles.baseline = ...
                handles.ardy.get_baseline_float(handles.d_index);           %Read in the baseline (unpressed) value for the lever.
            handles.slope = handles.ardy.get_slope_float(handles.d_index);  %Read in the slope value for the lever.
        end        
    ...
    case 'pull'                                                             %If the current device is the pull...
        if handles.ardy.version < 200                                       %If the controller microcode version is less than 2.00...
            ...
            if isnan(handles.slope) || round(1000*handles.slope) == 0       %If the slope is NaN or zero.
                handles.slope = 0.5;                                        %Arbitrarility set the slope to 0.5
                handles.ardy.set_slope_float(6,handles.slope);              %Save the arbitraty slope back to the controller.
            end
            if isnan(handles.baseline)                                      %If the baseline is NaN.
                handles.baseline = 100;                                     %Arbitrarility set the baseline to 0.5
                handles.ardy.set_baseline_float(6,handles.baseline);        %Save the arbitraty baseline back to the controller.
            end
        end
        fprintf(1,'handles.slope = %1.3f\n',handles.slope);                 %Print the slope.
        fprintf(1,'handles.baseline = %1.3f\n',handles.baseline);           %Print the baseline.
...
pause(0.025);                                                               %Pause for 25 milliseconds.
...
if datenum(version('-date')) >= 737685 && use_serialport == 1               %If the temp whileversion is 2019b or newer...    
    flush(serialcon);                                                       %Clear the input and output buffers.
else                                                                        %Otherwise, for older versions...
...
cue_tone_on = 0;                                                            %Create a flag to                                                   
if h.ardy.version >= 2.00 && h.stage(h.cur_stage).tones_enabled == 1        %If the controller sketch is version 2.0+ and tones are enabled...    
    ...
    for i = length(temp):-1:1                                               %Step backwards through the tones.
        switch lower(temp{i})                                               %Switch between the recognized tone initiation event types.
            ...
            case 'cue'                                                      %If the tone initiation event is a trial cue...
                if ~isempty(h.stage(h.cur_stage).tones(i).thresh) && ...
                        ~ischar(h.stage(h.cur_stage).tones(i).thresh) && ...
                        h.stage(h.cur_stage).tones(i).thresh(1) > 0
                    session.cue_tone_index = i;                             %Save the cue tone index.                
                    cue_tone_on = 1;                                        %Set the cue tone flag to 1.
                    cue_tone_period = ...
                        h.stage(h.cur_stage).tones(i).thresh/86400;         %Set the cue tone period, the time between the end of one trial and the next.
                    cue_tone_time = now + cue_tone_period;                  %Set the timing of the first cue tone.
                end
        end
    end
end
...
switch lower(h.device)                                                      %Switch between the recognized devices...
    ...
    case 'pull'                                                             %If the current device is the isometric pull...
        session.min_peak_val = h.threshmin;                                 %A "pull" must  exceed the minimum threshold.      
        session.lever_return_pt = h.init;                                   %The force must return to the initiation threshold before a new press begins.
end
...
%For random feeding modes, set the first random feeding time.
if ~isempty(h.random_feed)                                                  %If random feeding is enabled...
    temp = h.random_feed(1) + rand*(h.random_feed(2) - h.random_feed(1));   %Calculate the interval until the next random feeding.
    session.next_rand_feed = now + temp/86400;                              %Set the next random feeding time.
end
...
if isfield(h,'variant')                                                     %If a hit was scored on this loop and there's a custom variant in the handles structure...
    switch lower(h.variant)                                                 %Switch between the recognized custom variants.
        case 'hollis'                                                       %If this is a custom stage for the Hollis lab variant...
            h.ardy.lights(1);                                               %Turn on the cage lights.
            pause(0.25);                                                    %Pause for 0.25 seconds.
            h.ardy.lights(0);                                               %Turn off the cage lights.
    end
end    
...
while fix(run) == 2                                                         %Loop until the user ends the session.
    ...
    fprintf(1,'h.ceiling = %1.1f\n',h.ceiling);
    fprintf(1,'trial.ceiling_check = %1.0f\n',trial.ceiling_check);
...
%WAITING FOR TRIAL INITIATION ******************************************************************************************************
    while max(trial.mon_signal) < h.init && ...
        ...
        if strcmpi(h.stim,'random')                                         %If random stimulation is enabled...
        ...
        if cue_tone_on > 0 && now > cue_tone_time                           %If a cue tone is enabled and it's time to present the cue...
            h.ardy.play_tone(session.cue_tone_index);                       %Play the cue tone.
            cue_tone_time = cue_tone_time + cue_tone_period;                %Set the next cue tone time.
        end
...
%STARTING TRIAL ********************************************************************************************************************
    if run == 2                                                             %If the session is running and not paused or set for a manual feeding...
        ...
        if strcmpi(session.init_trig,'on') || strcmpi(h.stim,'initiation')  %If an initiation trigger is enabled...
            if isfield(h.ardy,'send_trigger')                               %If this is the newer function structure...
                h.ardy.send_trigger(1);                                     %Trigger stimulation through the MotoTrak controller.
            else                                                            %Otherwise...
                h.ardy.stim();                                              %Use the older stimulate function.
            end
        end
...
%RECORD TRIAL RESULTS **************************************************************************************************************
    ...
    elseif strcmpi(h.stim,'random') && fix(run) ~= 2 && ...
            ~isempty(trial.stim_time)                                       %If the user's stopped the session and random stimulation is enabled and there's stimulation times to write...
    ...
    if ~isempty(h.random_feed) && (now > session.next_rand_feed)            %If random feeding is enabled and it's time to deliver a random feeding...
        run = 2.2;                                                          %Set the run variable to 2.2.
        temp = h.random_feed(1) + ...
            rand*(h.random_feed(2) - h.random_feed(1));                     %Calculate the interval until the next random feeding.
        session.next_rand_feed = session.next_rand_feed + temp/86400;       %Set the next random feeding time.
    end
        
    if cue_tone_on > 0                                                      %If a cue tone is enabled...
        cue_tone_time = now + cue_tone_period;                              %Set the next cue tone time.
    end
end
...
%   04/29/2022 - Drew Sloan - Added a list of recognized stimulation
%       options.
% 
...
handles.stim = 'off';                                                       %Disable stimulation by default.
handles.pre_trial_sampling = 1;                                             %Set the pre-trial sampling period, in seconds.
handles.default_post_trial_sampling = 2;                                    %Set the default post-trial sampling period, in seconds.
...
handles.stim_options = {'OFF','HIT','INITIATION','BURST','RANDOM'};         %Set the stimulation options.
...
switch lower(handles.stim)                                                  %Switch between the stimulation types.
    case 'hit'                                                              %Stimulate on hits.
        set(handles.popvns,'foregroundcolor',[1 0 0]);                      %Make the stimulation triggers pop-up menu "ON" text red.
    case 'random'                                                           %Stimulate randomly.
         set(handles.popvns,'foregroundcolor',[0 0 1]);                     %Make the stimulation triggers pop-up menu "RANDOM" text blue.
    case 'initiation'                                                       %Stimulate on trial initiation.
        set(handles.popvns,'foregroundcolor',[0 1 0]);                      %Make the stimulation triggers pop-up menu "BURST" text green.
    case 'burst'                                                            %Burst stimulation.
    	set(handles.popvns,'foregroundcolor',[1 1 0]);                      %Make the stimulation triggers pop-up menu "BURST" text green.
    case 'off'                                                              %Stimulation off.
        set(handles.popvns,'foregroundcolor','k');                          %Make the stimulation triggers pop-up menu "OFF" text black.
end
i = strcmpi(handles.stim,handles.popvns.String);                            %Find the index for the stimulation type in the pop-up menu.
set(handles.popvns,'value',find(i));                                        %Update the pop-up menu.
...
%         handles.ardy.stream_enable(0);                                    %Disable streaming on the Arduino.
        if isfield(handles.ardy,'send_trigger')                             %If this is the newer function structure...
            handles.ardy.send_trigger(1);                                   %Trigger stimulation through the MotoTrak controller.
        else                                                                %Otherwise...
            handles.ardy.stim();                                            %Use the older stimulate function.
        end
%         handles.ardy.stream_enable(1);                                    %Re-enable streaming on the Arduino.
...
%             1023 - data(a:session.buffsize,3);                              %Copy the pre-trial touch values.
        ...
        if strcmpi(handles.stim,'hit')                                      %If stimulation is on...
...
%   04/29/2022 - Drew Sloan - Added handling for commented lines.
%
...
for i = 1:length(a) - 1                                                     %Step through all lines in the data.
    ...
    ln = strtrim(ln);                                                       %Trim the ends off the text.
    ln(ln == 0) = [];                                                       %Kick out all null characters.
    if isempty(ln) || ln(1) == '%'                                       %If this line is commented out...
...
%       ".stim".
%   04/29/2022 - Drew Sloan - Changed the *.stim field from numbered
%       indices to strings to be more intuitive.
%
...
switch lower(h.stage(h.cur_stage).stim)                                     %Switch between the different output trigger types.
    case {'on','hit'}                                                       %If the trigger type is "ON" or "HIT"...
        h.stim = 'hit';                                                     %Set the stim field to "hit".
    ...
    case 'random'                                                           %If triggers are randomly-presented by default for this stage...
        h.stim = 'random';                                                  %Set the stim field to "random".
        set(h.popvns,'foregroundcolor',[0 0 1]);                            %Make the stimulation triggers pop-up menu "RANDOM" text blue.
    case {'initiation','beginning of every trial'}                          %If the trigger type is "INITIATION" or "BEGINNING OF EVERY TRIAL"...
        h.stim = 'initiation';                                              %Set the stim field to "initiation".
        set(h.popvns,'foregroundcolor',[0 1 0]);                            %Make the stimulation triggers pop-up menu "BURST" text green.
    case 'burst'                                                            %If the trigger type is "BURST" or "INITIATION"...
        h.stim = 'burst';                                                   %Set the stim field to "BURST".
        set(h.popvns,'foregroundcolor',[1 1 0]);                            %Make the stimulation triggers pop-up menu "BURST" text green.
    otherwise                                                               %Otherwise, if triggers are disabled by default for this stage...
        h.stim = 'off';                                                     %Set the stim field to "off".
        set(h.popvns,'foregroundcolor','k');                                %Make the stimulation triggers pop-up menu "OFF" text black.
end
i = strcmpi(h.stim,h.popvns.String);                                        %Find the index for the stimulation type in the pop-up menu.
set(h.popvns,'value',find(i));                                              %Update the pop-up menu.
...
set(handles.popvns,'style','popup',...
    'string',upper(handles.stim_options),...
...
show_save = 0;                                                              %Create a timing variable for flashing a "Calibration Saved" message on the axes.
h.ardy.clear();                                                             %Clear any residual values from the serial line.
h.ardy.set_stream_period(10);                                               %Set the stream period on the Arduino to 10 milliseconds.
h.ardy.stream_enable(1);                                                    %Enable periodic streaming on the Arduino.
...
params = {  'stage number',                             'number',               'required',         [];... 
            ...
            'stimulation',                              'stim',                 'optional',         'OFF';...
            'vns default',                              'stim',                 'optional',         'OFF';...
...
for i = 1:length(stage)                                                     %Step through the stages.    
    ...
    if strcmpi(stage(i).device,'lever') && handles.d_index == 5             %If this is a lever stage and a V3.0 lever is connected...
        stage(i).stream_order(1) = ...
            inputs{strcmpi('force lever',inputs(:,1)),2};                   %Set the primary stream input to the potentiomter signal.
        stage(i).stream_order(2) = ...
            inputs{strcmpi('pull',inputs(:,1)),2};                          %Set the primary stream input to the potentiomter signal.
    end
end
...
trial.signal = zeros(session.buffsize,1);                                   %Reset out the trial signal.
...
if handles.stim == 1                                                        %If stimulation is enabled...
    ...
    trial.stim_time = now;                                                  %Save the current time as the hit time.
elseif strcmpi(handles.stim,'burst')                                        %If we are in burst stim mode...                                
...
%       "refresh" ability.
%   04/28/2021 - Drew Sloan - Transitioned code to use the new "serialport"
%       interface.
%
...
port = serialportlist('all');                                               %Grab all serial ports.
...
busyports = setdiff(port,serialportlist('available'));                      %Find all ports that are currently busy.
...
reset = 0;
while ~isempty(port) && reset == 0 &&...
        (length(port) > 1 || isempty(setdiff(port,busyports))) && ...
    ...
    if ishandle(fig1)                                                       %If the user didn't close the figure without choosing a port...
        ...
        if i > size(booth_pairings,1)                                       %If the user selected to refresh the list...
            ...
            reset = 1;                                                      %Set the reset flag to 1.
        end        
...
booth_pairings(:,1) = cellstr(port);                                        %Copy the available ports to the booth-port pairing cell array.
for i = 1:length(port)                                                      %Step through each available serial port.
    ...
    if strcmpi(busyports,port{i})                                           %If the port is currently busy...
        ...
        try                                                                 %Try to open the serial port for communication.     
            ...
            serialcon = serialport(port{i},115200);                         %Set up the serial connection on the specified port.
            write(serialcon,'A','char');                                    %Send the check status code to the Arduino board.
            timeout = now + 3/86400;                                        %Set a time-out point for the following loop.
            while now < timeout && serialcon.NumBytesAvailable == 0         %Loop for 3 seconds or until the Arduino initializes.
                pause(0.01);                                                %Pause for 10 milliseconds.
            end
            if serialcon.NumBytesAvailable > 0                              %If there's a reply on the serial line.
                flush(serialcon);                                           %Clear the input and output buffers.
                write(serialcon,'A','char');                                %Send the check status code to the Arduino board.
                timeout = now + 3/86400;                                    %Set a time-out point for the following loop.
                while now < timeout && serialcon.NumBytesAvailable == 0     %Loop for 3 seconds or until the Arduino initializes.
                ...
                if serialcon.NumBytesAvailable > 0                          %If there's a reply on the serial line.
                    ...
                    if any(temp(end) == [111, 123])                         %If the reply is the "111" or "123" expected...     
                        write(serialcon,'BA','char');                       %Send the command to the Arduino board.
                        temp = readline(serialcon);                         %Read in the incoming data.
                        temp = sscanf(temp,'%f')';                          %Convert the text to numbers.
                        booth = temp(1);                                    %Keep only the first number.
                    end
...
%% ***********************************************************************
function [port, booth_pairings] = ...
    MotoTrak_Select_Serial_Port_Deprecated(booth_pairings)

%MotoTrak_Select_Serial_Port_Deprecated.m - Vulintus, Inc., 2021
%
%   MotoTrak_Select_Serial_Port_Deprecated is the deprecated version of
%   MotoTrak_Select_Serial_Port, which detects available serial ports for
%   MotoTrak systems and compares them to serial ports previously 
%   identified as being connected to MotoTrak systems. This deprecated
%   version is being kept active for the benefit of users running pre-2019b
%   MATLAB.
%
%   UPDATE LOG:
%   05/09/2016 - Drew Sloan - Separated serial port selection from
%       Connect_MotoTrak function to enable smarter port detection and list
%       "refresh" ability.
%   04/27/2021 - Drew Sloan - Deprecated function copied from
%       MotoTrak_Select_Serial_Port.
%
...
if isempty(booth_pairings)                                                  %If there are no existing booth pairings...
    booth_pairings = Poll_Available_Ports_deprecated(port,busyports);       %Call the subfunction to check each available serial port for a MotoTrak system.
    if isempty(booth_pairings)                                              %If no MotoTrak systems were found...
        errordlg(['ERROR: No MotoTrak Systems were detected on this '...
            'computer!'],'No MotoTrak Connections!');                       %Show an error in a dialog box.
        port = [];                                                          %Set the function output to empty.
...
while ~isempty(port) && length(port) > 1 && ...
    ...
    if ishandle(fig1)                                                       %If the user didn't close the figure without choosing a port...
        ...
        if i > size(booth_pairings,1)                                       %If the user selected to refresh the list...
            booth_pairings = ...
                Poll_Available_Ports_deprecated(port,busyports);            %Call the subfunction to check each available serial port for a MotoTrak system.
            if isempty(booth_pairings)                                      %If no MotoTrak systems were found...
                errordlg(['ERROR: No MotoTrak Systems were detected on '...
                    'this computer!'],'No MotoTrak Connections!');          %Show an error in a dialog box.
                port = [];                                                  %Set the function output to empty.
...
%% This subfunction steps through available serial ports to identify MotoTrak connections.
function booth_pairings = Poll_Available_Ports_deprecated(port,busyports)
waitbar = big_waitbar('title','Polling Serial Ports');                      %Create a waitbar figure.
booth_pairings = cell(length(port),2);                                      %Create a cell array to hold booth-port pairings.
...
for i = 1:length(port)                                                      %Step through each available serial port.
    ...
    if strcmpi(busyports,port{i})                                           %If the port is currently busy...
        ...
        try                                                                 %Try to open the serial port for communication.     
            ...
            serialcon.Timeout = 1;                                          %Set the timeout duration to 1 seconds.
            fopen(serialcon);                                               %Open the serial port.    
            booth_pairings{i,1} = 'delete';                                 %Assume at first that the port will be excluded.            
            fwrite(serialcon,'A','uchar');                                  %Send the check status code to the Arduino board.
            timeout = now + 5/86400;                                        %Set a time-out point for the following loop.
            while now < timeout && serialcon.BytesAvailable == 0            %Loop for 5 seconds or until the Arduino initializes.
                pause(0.01);                                                %Pause for 10 milliseconds.
            end
            if serialcon.BytesAvailable > 0                                 %If there's a reply on the serial line.
                ...
                if serialcon.BytesAvailable > 0                              %If there's a reply on the serial line.
                    pause(0.01);                                            %Pause for 10 milliseconds.
                    temp = fscanf(serialcon,'%d');                          %Read the reply.          
                    booth = [];                                             %Create a variable to hold the booth number.
                    if any(temp(end) == [111, 123])                         %If the reply is the "111" or "123" expected...     
                        fwrite(serialcon,'BA','uchar');                     %Send the command to the Arduino board.
                        booth = fscanf(serialcon,'%d');                     %Read the reply.
                        booth = booth(1);                                   %Keep only the first number.
                        booth = num2str(booth,'%1.0f');                     %Convert the booth number to a string.      
                    end
                    if ~isempty(booth)                                      %If a booth number was returned...
                        booth_pairings{i,1} = port{i};                      %Save the port number.
...
if isfield(handles,'custom')                                                %If the handles structure has a custom field...
    switch lower(handles.custom)                                            %Switch between the various recognized variants.
        case {'machado lab', 'touch/pull'}                                  %Touch/Pull customization.        
            if strcmpi(handles.stim,'initiation') && ...
...
if handles.ardy.version >= 200                                              %If the controller sketch version is 2.00 or newer...
...
switch handles.curthreshtype                                                %Switch between the types of hit threshold.
    ...
    case {'presses', 'fullpresses', 'pulls'}                                %If the current threshold type is presses (for LeverHD) or pulls...
        if strcmpi(handles.device, 'knob')
              trial.mon_signal(new_samples) = ...
...
%% ***********************************************************************
function [session, trial] = MotoTrak_Update_Trial_Signal(handles,session,trial)
...
%   11/18/2021 - Drew Sloan - Removed the output variable "N" and changed
%       all references to "N" to "trial.N" to use the trial structure. This
%       change was precipitated by a bug in over-under behavior tasks.
%
...
if trial.cur_sample + trial.N > session.buffsize                            %If more samples were read than we'll record for the trial...
    trial.N = session.buffsize - trial.cur_sample;                          %Pare down the read samples to only those needed.
end

new_samples = trial.cur_sample+(1:trial.N);                                 %Grab the indices for the new samples.

trial.data(new_samples,:) = new_data(1:trial.N,:);                          %Add the new samples to the trial data.    

switch handles.curthreshtype                                                %Switch between the types of hit threshold.

    case 'bidirectional'                                                    %If the current threshold type is the bidirectional number of degrees...
        trial.signal(trial.cur_sample+(1:trial.N)) = ...
    ...
    case {  'grams (peak)',...
        ...
        trial.cur_val = trial.signal(trial.cur_sample + trial.N);           %Grab the current value.
    case {'degrees/s','# of spins'}                                         %If the current threshold type is the number of spins or spin velocity.
        temp = diff(session.buffer(:,2));                                   %Find the wheel velocity at each point in the buffer.
        temp = boxsmooth(temp,session.min_peak_dist);                       %Boxsmooth the wheel velocity with a 100 ms smoothandles.            
        trial.signal(trial.cur_sample+(-session.offset:trial.N)) = ...
                temp(session.buffsize-trial.N-1-session.offset:session.buffsize-trial.N+trial.N-1);       %Find the wheel velocity thus far in the trial.
    ...
    case 'milliseconds/grams'                                               %If the current threshold type is a hold...
        ...
        i = trial.cur_sample + trial.N;                                     %Grab the current sample.
        if trial.hit_time == 0 && ...
                any(trial.touch_signal(new_samples) > 511.5)                %If the rat went back to the touch sensor...
            trial.buffsize = trial.cur_sample + trial.N;                    %Set the new buffer timeout.
            trial.hit_time = -1;                                            %Set the hit time to -1 to indicate an abort.
        elseif strcmpi(handles.stim,'burst') && ...
                any(trial.signal >= 5) && ...
...
switch handles.curthreshtype                                                %Switch between the types of hit threshold.
    ...
    case {'presses', 'fullpresses'}                                         %For lever press count thresholds...
        ...
        presses_signal = trial.signal(1:trial.cur_sample+trial.N) - session.min_peak_val;
        ...
        trial.peak_indices = ...
            original_indices(find(diff_presses_signal == 1))';
        ...
        trial.peak_vals = [];
        trial.peak_vals(1:length(trial.peak_indices)) = ...
            session.min_peak_val;
...
i = find(trial.peak_indices >= session.pre_samples & trial.peak_vals >= 1 &...
    ...
    trial.peak_indices <= trial.cur_sample + trial.N - session.offset );    %Find all of the of peaks in the hit window.
br = find(trial.release_points >= session.pre_samples & rpks >= 1 & ...
    trial.release_points < session.pre_samples + session.hit_samples & ...
    trial.release_points <= trial.cur_sample + trial.N - session.offset );
%                 rpks = rpks(br);
...
switch lower(handles.stim)                                                  %Switch between the stimulation types.
    ...
    case 'initiation'                                                       %Stimulate on trial initiation.
        stim = 'Stim';                                                      %Show that there's stimulation in the filename.
    case 'burst'                                                            %Burst stimulation.
    	stim = 'BurstStim';                                                 %Show that there's burst in the filename.
    case 'off'                                                              %Stimulation off.
...
%   04/26/2022 - Drew sloan - Replaced "sprintf('\n')" with MATLAB's
